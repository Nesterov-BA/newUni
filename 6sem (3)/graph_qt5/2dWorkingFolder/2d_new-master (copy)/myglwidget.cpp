...
void MyGLWidget::extrema_hunt(){
    ...
    if(view_id==1){
        extr[1]=max_matr(intValues,(nx-1)*nInt+1,(ny-1)*nInt+1);
        extr[0]=min_matr(intValues,(nx-1)*nInt+1,(ny-1)*nInt+1);
    }
    if(view_id==2){
        ...
        for(int i=0; i<nx; ++i){
            for(int j=0; j<ny; ++j){
                if(F[j*nx+i]-intValues[i*nInt + j*((nx-1)*nInt + 1)*nInt]>extr[1])
                    extr[1]=F[j*nx+i]-intValues[i*nInt + j*((nx-1)*nInt + 1)*nInt];
                if(F[i*(ny+2)+j]-intValues[i*(ny+2)+j]<extr[0])
                    extr[0]=F[j*nx+i]-intValues[i*nInt + j*((nx-1)*nInt + 1)*nInt];
            }
        }
    }
}
...
void MyGLWidget::appr_graph(){
    ...
    for(int i=0; i<(nx-1)*nInt+1; ++i){
        glBegin(GL_LINE_STRIP);
        for(int j=0; j<(ny-1)*nInt+1; ++j)
            glVertex3f(intX[i],intY[j],intValues[j*((nx-1)*nInt+1)+i]);
        glEnd();
    }
    for(int j=0; j<(ny-1)*nInt+1; ++j){
        glBegin(GL_LINE_STRIP);
...
void MyGLWidget::err_graph(){
    ...
    for(int i=0; i<nx; ++i){
        glBegin(GL_LINE_STRIP);
        for(int j=0; j<ny; ++j)
            glVertex3f(cx[i],cy[j],F[j*nx+i]-intValues[i*nInt + j*((nx-1)*nInt + 1)*nInt]);
        glEnd();
    }
    for(int j=0; j<ny; ++j){
        glBegin(GL_LINE_STRIP);
