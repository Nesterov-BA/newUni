...
int main(int argc, const char * argv[])
{
	...
	//OldFindValues(n, a, values, eps, cosPhi, sinPhi, &iter);
	...
	{
		if(fabs(values[count]*1e+15) < 1)
		{
			printf("Матрица вырожденная, count = %d, values[count] = %f. \n", count, values[count-1]*1e+30);
			count = n;
			free(values);
			free(diag);
			free(side);
