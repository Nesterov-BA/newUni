...
void interpolatetedVal(double* gamma, int xInt, int yInt, double* intVal, double xStart, double xEnd, double yStart, double yEnd, int intPointsNum, int nX, int nY)
{
    ...
    double xStep = (xEnd - xStart)/intPointsNum;
    double yStep = (yEnd - yStart)/intPointsNum;
    double* xIntPoints = new double[intPointsNum];
    double* yIntPoints = new double[intPointsNum];
    for(int i = 0; i < intPointsNum; i++)
    {
        xIntPoints[i] = xStart + xStep*i;
        yIntPoints[i] = yStart + yStep*i;
    }
    for (int i = 0; i < intPointsNum; i++)
    {
        for (int j = 0; j < intPointsNum; j++)
        {
            basis(xIntPoints[i], xIntPoints[j], xStart, yStart, basFuncs);
            for(int k = 0; k < 16; k++)
                intVal[yInt*nX*intPointsNum*intPointsNum + xInt*intPointsNum*intPointsNum + i*intPointsNum + j] += gamma[xInt*nY*16 + yInt*16 + k]*basFuncs[k];
        }
    }
    delete [] basFuncs;
    delete [] xIntPoints;
