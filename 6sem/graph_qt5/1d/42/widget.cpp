...
void Widget::plot(QWidget *parent) {
    ...
    double* coeff = new double[3*(n+1)];
    coefficents_eval(x_, fValues, n+1, func_id, coeff, h);

    double* evalPoints = new double[k*n+1];
    double* evalValues = new double[k*n+1];
    h = h/k;
    for (int j = 0; j < k * n+1; j++){
        evalPoints[j] = a + h*j - k*h/2;
    }
    for (int j = 0; j < n; j++){
        for (int i = 0; i < k; i++)
        {
            evalValues[k*j+i] = approx_eval(evalPoints[k*j+i], coeff, j, ksi);
        }
    }

    evalValues[k*n] = approx_eval(evalPoints[k*(n-1)], coeff, n-1, ksi);
    ...
    axisY->setRange(fmin(fValues[0], 0), fmax(fValues[n-1],1));
